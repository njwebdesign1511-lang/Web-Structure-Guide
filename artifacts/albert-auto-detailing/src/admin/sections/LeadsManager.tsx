    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { icon: Phone, label: "Recientes", val: stats?.recent?.length ?? "—", color: "#1460a0" },
          { icon: Car, label: "Con vehículo", val: stats?.recent?.filter(l => l.vehicle)?.length ?? "—", color: "#4d8fd4" },
          { icon: Mail, label: "Con email", val: stats?.recent?.filter(l => l.email)?.length ?? "—", color: "#1460a0" },
                ...
                <div className="border-t border-white/5 p-4 grid sm:grid-cols-2 gap-3">
                  {[
                    ...
                    <div key={label} className="bg-[#0a0d14] rounded-lg p-3">
                      ...
                      {href ? (
                        <a href={href} className="text-sm font-medium" style={{ color: "#4d8fd4" }}>{val}</a>
                      ) : (
